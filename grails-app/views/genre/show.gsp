	...
	<body>
		...
		<div id="show-genre" class="content scaffold-show" role="main">
			...
			<ol class="property-list genre">
				...
				<g:if test="${genreInstance?.events}">
				<li class="fieldcontain">
					<span id="events-label" class="property-label"><g:message code="genre.events.label" default="Events" /></span>
					
						<g:each in="${genreInstance.events}" var="e">
						<span class="property-value" aria-labelledby="events-label"><g:link controller="eventGenre" action="show" id="${e.id}">${e?.encodeAsHTML()}</g:link></span>
						</g:each>
